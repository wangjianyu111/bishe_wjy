<template>
  <div class="proposal-page">
      ...
      <el-card shadow="never" class="history-card">
        <template #header>
          <div class="card-header">
            <span>提交记录</span>
            <span v-if="myProposalList.length" class="record-count">共 {{ myProposalList.length }} 条</span>
          </div>
        </template>
        <el-table :data="myProposalList" border stripe size="small" v-loading="loadingMyData">
          ...
          <el-table-column label="状态" width="100">
            <template #default="{ row }">
              <el-tag size="small" :type="proposalTagType(row.status)">
        ...
        <el-empty v-if="!loadingMyData && !myProposalList.length" description="暂无提交记录" style="padding: 40px 0" />
      </el-card>
    </template>
    ...
    <!-- ===== 学生提交对话框 ===== -->
    <el-dialog v-model="submitDialogVisible" title="提交开题报告" width="600px" destroy-on-close>
    ...
    <!-- ===== 查看详情对话框 ===== -->
    <el-dialog v-model="detailVisible" title="开题报告详情" width="640px" destroy-on-close>
      <el-descriptions :column="2" border size="small">
        ...
        <el-descriptions-item label="状态">
          <el-tag size="small" :type="proposalTagType(currentRow?.status)">
            {{ proposalStatusText(currentRow?.status) }}
          </el-tag>
        </el-descriptions-item>
    ...
    <!-- ===== 驳回原因对话框 ===== -->
    <el-dialog v-model="rejectDialogVisible" title="驳回开题报告" width="500px" destroy-on-close>
...
import {
  ...
  submitProposal,
  recallProposal,
  fetchAdminProposalPage,
  fetchTeacherProposalPage,
  reviewProposal,
  downloadDocFile,
  fetchMyProposalList,
} from '@/api/project'
...
async function loadMyData() {
  ...
  try {
    ...
    if (!mySelection.value) {
      myProposalList.value = []
      return
    }
    try {
      const res = await fetchMyProposalList()
      myProposalList.value = Array.isArray(res) ? res : []
    } catch {
      myProposalList.value = []
    }
...
function handleStudentSubmitClick() {
  ...
  if (st === 'PASSED') {
    ElMessage.warning('您已通过开题报告审核，无需再次上传')
    return
  }
  if (st === 'PENDING') {
...
function proposalTagType(status) {
...
const submitRules = {
  reportContent: [
    { required: true, message: '请填写开题报告内容', trigger: 'blur' },
...
async function handleSubmit() {
  ...
  try {
    ...
    ElMessage.success('开题报告提交成功，已通知指导教师')
...
async function handleRecall() {
  const latest = myProposalList.value[0]
  if (!latest?.proposalId) return
  try {
    await ElMessageBox.confirm('确定要撤回开题报告吗？撤回后可重新提交。', '确认撤回', {
      confirmButtonText: '撤回',
      cancelButtonText: '取消',
      type: 'warning',
    })
    await recallProposal(latest.proposalId)
...
async function loadData(page = 1) {
  ...
  try {
    ...
    let res
    if (isAdmin.value) {
      res = await fetchAdminProposalPage(params)
    } else {
      res = await fetchTeacherProposalPage(params)
    }
...
async function confirmReject() {
  ...
  try {
    await reviewProposal({
      proposalId: pendingReviewRow.value.proposalId,
      status: 'FAILED',
      inspectComment: rejectForm.inspectComment,
    })
    ElMessage.success('已驳回开题报告，已通知学生')
